					...
					<div class="col-lg-9">
						...
						<div class="shop_content">
							<div class="shop_bar clearfix">
								<div class="shop_product_count"><span>{{$result}}</span> products found in <span>{{ ucwords($selected_store[0]['store_name']) }}</span></div>
								...
								@foreach($products as $product)
									...
									<div class="product_item is_new">
										...
										<div class="product_image d-flex flex-column align-items-center justify-content-center">
											@guest
												<a href="/view_product_guest/{{$product->id}}/info">
													...
													<img src="{{$product->getPicture($product->id)}}" alt="" class="expand">
												</a>
											@else
										...
										<div class="product_content">
											...
											<div style="margin-top: 5%; color:red;" class="product_price">
												{{ number_format( $latestprice[0]['product_price'],2 ) }}
												{{-- @if($minprice == $maxprice)
													{{number_format($minprice,2)}}
												@else
													{{number_format($minprice,2)}} ~ P{{number_format($maxprice,2)}}
												@endif  --}}
											</div>
												...
												<div style="display: none">
													{!!
														$latestprice = App\PostDetails::where('product_id', $rel->product_id)
																	->orderBy('created_at', 'desc')->limit(1)->get();
														$minprice = App\PostDetails::where('product_id', $rel->product_id)->min('product_price');       
														$maxprice = App\PostDetails::where('product_id', $rel->product_id)->max('product_price');	
													!!}
												</div>
